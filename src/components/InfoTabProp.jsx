...
export default function InfoTabsProp() {
  const defaultTab = "adminDashboard";
  const [active, setActive] = useState(defaultTab);
  const id = useId();

  const tabs = useMemo(() => ([
    { key: "adminDashboard", label: "Prop Firm Dashboard", Icon: Cog6ToothIcon },
    { key: "ibPortal",    label: "Challenge & Evaluation Portal", Icon: PuzzlePieceIcon },
    { key: "security",   label: "Risk & Security Controls",  Icon: UserCircleIcon },
    { key: "roles",      label: "Team Roles & Access", Icon: ShieldCheckIcon },
    { key: "client",     label: "Trader Area",       Icon: Squares2X2Icon },
    { key: "ticket",    label: "Dispute & Support Center",      Icon: UsersIcon },
  ]), []);
  ...
  return (
    <section className='relative flex flex-col items-center gap-7 px-4 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-700 dark:text-white'>
      {/* Mobile select */}
      <Title title="Experience the Prop CRM Workflow" desc="Dive into the essential tools built for prop firms. From challenge management to payouts and security, explore how our CRM powers every stage of your funded trader programs."/>
