...
const PANELS = {
  adminDashboard: (
    <div className="w-full">
      <img src={assets.banner_crm} className="w-full h-[500px] object-cover" />
    </div>
  ),
  ibPortal: (
    <div className="w-full">
      <img src={assets.ib_crm} className="w-full h-[500px] object-cover" />
    </div>
  ),
  security: (
    <div className="w-full">
      <img src={assets.two_fac_crm} className="w-full h-[500px] object-cover" />
    </div>
  ),
  roles: (
    <div className="w-full">
      <img src={assets.role_mang_crm} className="w-full h-[500px] object-cover" />
    </div>
  ),
  client: (
    <div className="w-full">
      <img src={assets.lang_crm} className="w-full h-[500px] object-cover" />
    </div>
  ),
  ticket: (
    <div className="w-full">
      <img src={assets.ticket_crm} className="w-full h-[500px] object-cover" />
    </div>
  ),
};


export default function InfoTabPamm() {
  const defaultTab = "adminDashboard";
  const [active, setActive] = useState(defaultTab);
  const id = useId();

  const tabs = useMemo(() => ([
    { key: "adminDashboard", label: "Fund Management Dashboard", Icon: Cog6ToothIcon },
    { key: "ibPortal",    label: "Investor & IB Portal", Icon: PuzzlePieceIcon },
    { key: "security",   label: "Compliance & Security",  Icon: UserCircleIcon },
    { key: "roles",      label: "Manager Roles & Controls", Icon: ShieldCheckIcon },
    { key: "client",     label: "Investor Area",       Icon: Squares2X2Icon },
    { key: "ticket",    label: "Support & Communication Hub",      Icon: UsersIcon },
  ]), []);
  ...
  return (
    <section className='relative flex flex-col items-center gap-7 px-4 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-700 dark:text-white'>
      {/* Mobile select */}
      <Title title="See PAMM/MAM CRM in Motion" desc="Discover how our CRM supports money managers and investors. From trade allocation to investor reporting, explore the modules that simplify account management and boost transparency."/>
